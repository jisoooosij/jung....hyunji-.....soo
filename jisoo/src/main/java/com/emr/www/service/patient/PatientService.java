...
public class PatientService {
	...
	@Transactional
	public PatientEntity registerPatient(PatientDTO patientDTO) {
...
//        patient.setId(null);
	...
	public PatientDTO getPatientBySecurityNum(String securityNum) {
		PatientEntity patient = patientRepository.findBySecurityNum(securityNum);
		if (patient != null) {
...
//	}
    ...
    private String getUniqueName(String name) {
        ...
        List<PatientEntity> existingPatients = patientRepository.findByNameStartingWith(name);

        int maxSuffix = 0;

        // 동일한 이름을 가진 기존 환자들에 대해 (1), (2), (3) 등의 방법으로 이름 생성
        // 기존 환자들의 이름을 순회하며 사용된 최대 접미사를 계산합니다.
        for (PatientEntity existingPatient : existingPatients) {
